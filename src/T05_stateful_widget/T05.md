...
- The `State` class has its own lifecycle methods, such as `initState()` and `setState()`.
...
- Keys are used to preserve the state of widgets when they are moved in the widget tree.
- Keys are especially important for stateful widgets to ensure that the correct state is associated with the correct widget.

---

# Types of Keys

- `ValueKey`: used to identify widgets based on a specific value.
- `UniqueKey` : generates a unique key for each widget instance.
- `ObjectKey`: uses the identity of an object to identify the widget.
- `GlobalKey`: used to access a widget's state from outside its own context.
...
- [Life cycle of a RenderObject](https://www.youtube.com/watch?v=PONvNV3u1Wg)
- [How Flutter Works: The State class](https://www.youtube.com/watch?v=FP737UMx7ss)
